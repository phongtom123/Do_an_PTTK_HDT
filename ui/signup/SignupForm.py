...
from logic.user import User, UserManager
...
class SignupForm:
    ...
    def signup(self):
        ...
        # TH phải query
            ...
            user_mgr = UserManager.UserManager()

            if user_mgr.check_email(email) == True:
                messagebox.showinfo("Hỏng", "Email đã tồn tại.")
            elif user_mgr.check_user_name(username) == True:
                ...
                new_user = User.User(
                    ...
                    user_email = email,
                    user_rank = 0,
                    user_status = 1, # 1 là đang hoạt động
                    user_role_id = 2 # 2 là user
                )
                is_success = user_mgr.add_user(new_user)
    ...
    def get_back_login(self):
        self.window.destroy()
        from ui.login.LoginForm import LoginForm
        new_window = tk.Tk()
